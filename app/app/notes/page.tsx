...
export default async function NotesPage({
  ...
  const subjectName = new Map(
    (q.trim() ? allSubjects : subjects).map((subject) => [subject.id, subject.name]),
  );
  const last = recent[0];
  const ordered = [
  ...
  const dumpSubjects = q.trim() ? allSubjects : subjects;
      ...
      <div className="space-y-3">
        ...
        {ordered.length === 0 ? (
          <p className="text-muted-foreground">
            {q.trim()
              ? "No subjects match."
              : "No subjects yet. Create one below, or dump a thought into Inbox."}
          </p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2">
            {ordered.map((subject) => (
